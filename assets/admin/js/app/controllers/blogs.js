...
app.controller('blogsCtrl', ['$scope', 'blogsService', '$rootScope', '$window', function ($scope, blogsService, $rootScope, $window) {
        $scope.listData = [];
        ...
        $scope.getBlogs = function () {
            ...
            blogsService.getList(reqData).then(function (response) {
                $scope.listData = [];
                ...
                if (response.ResponseCode == 200) {
                    ...
                    if ($scope.noOfObj == 0) {
                        ...
                        $('#blogsCtrl table>tbody').append('<tr id="noresult_td"><td colspan="7"><div class="no-content text-center"><p>' + ThereIsNoRecordToShow + '</p></div></td></tr>');
                    ...
                    $('.download_link,#selectallbox').hide();
                    $('#blogsCtrl table>tbody').append('<tr id="noresult_td"><td center" colspan="7"><div class="no-content text-center"><p>' + response.Message + '</p></div></td></tr>');
                    $('.simple-pagination').hide();
                }
                hideLoader();
        ...
        $scope.remove_flag = function () {
            var reqData = {
                EntityType: 'Blog',
                AdminLoginSessionKey: $scope.AdminLoginSessionKey,
                EntityGUID: $scope.data.blog_guid
            }
            blogsService.removeFlag(reqData).then(function (response) {
        ...
        $scope.delete_blog = function () {
            var reqData = {
                AdminLoginSessionKey: $scope.AdminLoginSessionKey,
                BlogGuID: $scope.data.blog_guid
            }
            showLoader();
            blogsService.deleteBlog(reqData).then(function (response) {
                hideLoader();
                closePopDiv('delete_popup', 'bounceOutUp');
                if (response.ResponseCode == 200) {
                    ShowSuccessMsg(response.Message, 'success');
        ...
        $scope.view_flag_details = function () {
            ...
            blogsService.viewFlags(reqData).then(function (response) {
        ...
        $scope.view_post = function(blog_guid){
            var url = base_url+'article/details/'+blog_guid;
...
$(document).ready(function () {
    ...
    $('#searchButtonA').on('click', function () {
        if ($('#searchButtonA').hasClass('selected')) {
            angular.element($('#blogsCtrl')).scope().getBlogs();
        } else {
            $('#searchButtonA').addClass('selected');
            $('.search-block').show();
        }
...
app.controller('deleteBlogsCtrl', ['$scope', 'blogsService', '$rootScope', '$window', function ($scope, blogsService, $rootScope, $window) {
        $scope.listData = [];
        ...
        $scope.getBlogs = function () {
            ...
            var reqUrl = reqData[1]
            //Call getUserlist in services.js file
            blogsService.getDeleteList(reqData).then(function (response) {
                $scope.listData = [];
                //If no. of records greater then 0 then show
                $('.download_link,#selectallbox').show();
                $('#noresult_td').remove();
