...
app.controller('mediaAbuseCtrl', function ($scope, mediaAbuseData, $rootScope, $document, $window, $filter, $sce) {
    $scope.mediaSummary = {};
    $scope.items = {};
    $scope.user = {};
    $scope.mediaListData = [];
            ...
            if (response.ResponseCode == 200) {
                ...
                if ($scope.mediastatus == 'delete') {
                    angular.forEach($scope.indexToUpdate, function (val, key) {
                        ...
                        $scope.getMediaSummary();
                        $scope.getSearchBox();
                        $scope.totalAbuseMedia = parseInt($scope.totalAbuseMedia)  - 1;
                    })
                } else if ($scope.mediastatus == 'approve') {
                    angular.forEach($scope.indexToUpdate, function (val, key) {
                        //$scope.mediaList[key].IsAdminApproved = 1;
                        $scope.mediaList.splice(key, 1);
                        $scope.getMediaSummary();
                        $scope.getSearchBox();
                        $scope.totalAbuseMedia = parseInt($scope.totalAbuseMedia)  - 1;
    ...
    $scope.mediaDetail = {};
    $scope.mediaVideoPopup = function (media)
    {
        $('#mediaVideoBox').html('');
        var tempSorce = '';
        $scope.mediaDetail = {};
        $scope.mediaDetail = media;
        openPopDiv('mediaVideoPopup', 'bounceInDown');
        if ($scope.mediaDetail.MediaType == 'Video')
        {
            var temp_name = $scope.mediaDetail.ImageName.split('.');
            temp_name.splice(temp_name.length - 1);
            temp_name = temp_name.join('')

            $scope.mediaDetail.videos = [
                {src: $sce.trustAsResourceUrl(image_server_path + 'uploads/album/' + temp_name + '.mp4'), type: "video/mp4"},
                {src: $sce.trustAsResourceUrl(image_server_path + 'uploads/album/' + temp_name + '.webm'), type: "video/webm"},
                {src: $sce.trustAsResourceUrl(image_server_path + 'uploads/album/' + temp_name + '.ogg'), type: "video/ogg"},
            ];
            
            var video_name = temp_name;
            var obj, source1, source2, source3 ;
            obj = document.createElement('video');
            $(obj).attr('id', 'obj');
            $(obj).attr('class', 'video-js vjs-default-skin');
            $(obj).attr('width', '320');
            $(obj).attr('data-height', '280');
            $(obj).attr('controls', ' ');
            $(obj).attr('preload', 'auto');
            $(obj).attr('data-setup', '{}');
            source1 = document.createElement('source');
            $(source1).attr('type', 'video/mp4');
            $(source1).attr('src', $sce.trustAsResourceUrl(image_server_path + 'uploads/album/' + video_name + '.mp4'));
            
            source2 = document.createElement('source');
            $(source2).attr('type', 'video/webm');
            $(source2).attr('src', $sce.trustAsResourceUrl(image_server_path + 'uploads/album/' + video_name + '.webm'));
            
            source3 = document.createElement('source');
            $(source3).attr('type', 'video/ogg');
            $(source3).attr('src', $sce.trustAsResourceUrl(image_server_path + 'uploads/album/' + video_name + '.ogg'));
            $("#mediaVideoBox").append(obj);
            $(obj).append(source1);
            $(obj).append(source2);
            $(obj).append(source3);

        }
        //document.getElementById("mediaVideo").load();
        return false;
    }
});
...
app.controller('mediaAbuseDetailCtrl', function ($scope, mediaAbuseData, $rootScope, $document, $window, $filter, $sce) {
    $scope.mediaDetail = {};
    $scope.MediaId = '';
    $scope.mediaComments = [];
    $scope.statusMediaIds = [];
        ...
        mediaAbuseData.getAbuserMediaDetail(reqData).then(function (response) {
            if (response.ResponseCode == 200) {
                ...
                if ($scope.mediaDetail.MediaType == 'Video')
                {
                    ...
                    $scope.videoSources = [{
                            src: image_server_path + 'uploads/album/' + temp_name + '.mp4',
                            type: "video/mp4",
                            media: "screen"
                        },
                        image_server_path + 'uploads/album/' + temp_name + '.mp4',
                        image_server_path + 'uploads/album/' + temp_name + '.webm',
                        image_server_path + 'uploads/album/' + temp_name + '.ogg',
                    ];
                }
                ;
            } else if (response.ResponseCode == 517) {
