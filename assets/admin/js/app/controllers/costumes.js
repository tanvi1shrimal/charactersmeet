...
app.controller('costumesCtrl', ['$scope', 'costumesService', '$rootScope', '$window', function ($scope, costumesService, $rootScope, $window) {
        $scope.listData = [];
        ...
        $scope.getCostumes = function () {
            ...
            costumesService.getList(reqData).then(function (response) {
                $scope.listData = [];
                ...
                if (response.ResponseCode == 200) {
                    ...
                    if ($scope.noOfObj == 0) {
                        ...
                        $('#costumeCtrl table>tbody').append('<tr id="noresult_td"><td colspan="7"><div class="no-content text-center"><p>' + ThereIsNoRecordToShow + '</p></div></td></tr>');
                    ...
                    $('.download_link,#selectallbox').hide();
                    $('#costumesCtrl table>tbody').append('<tr id="noresult_td"><td center" colspan="7"><div class="no-content text-center"><p>' + response.Message + '</p></div></td></tr>');
                    $('.simple-pagination').hide();
                }
                hideLoader();
        ...
        $scope.remove_flag = function () {
            var reqData = {
                EntityType: 'Article',
                AdminLoginSessionKey: $scope.AdminLoginSessionKey,
                EntityGUID: $scope.data.article_guid
            }
            costumesService.removeFlag(reqData).then(function (response) {
        ...
        $scope.delete_costume = function () {
            var reqData = {
                AdminLoginSessionKey: $scope.AdminLoginSessionKey,
                ArticleGuID: $scope.data.article_guid
            }
            costumesService.deleteArticle(reqData).then(function (response) {
                closePopDiv('delete_popup', 'bounceOutUp');
                if (response.ResponseCode == 200) {
                    ShowSuccessMsg(response.Message, 'success');
                    $scope.getCostumes();
        ...
        $scope.view_flag_details = function () {
            ...
            costumesService.viewFlags(reqData).then(function (response) {
...
$(document).ready(function () {
    ...
    $('#searchButtonA').on('click', function () {
        if ($('#searchButtonA').hasClass('selected')) {
            angular.element($('#costumeCtrl')).scope().getCostumes();
        } else {
            $('#searchButtonA').addClass('selected');
            $('.search-block').show();
        }
